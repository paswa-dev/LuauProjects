...
local module = {
	...
	springs = {
		...
		Shake = Spring.new(Vector3.zero, 3, 1),
		Aim = Spring.new(Vector3.zero, 5, 1)
	},
	events = {
		reload = Signal.new(),
		fire = Signal.new(),
...
function module:updateMath(dt)
	...
	springs.Shake.Target = Vector3.new(
		maxShake.X * CS * perlin(Seed, self.elapsed * T),
		maxShake.Y * CS * perlin(Seed+200, self.elapsed * T),
		0
	)
end
...
function module:updateArms(dt)
	...
	RCFrame *= CFrame.new(Springs.Aim.Value)
	RCFrame *= CFrame.new(Springs.Shake.Value)
...
function module:aimIn()
	if not self.aim then
		self.aim = true
		self.springs.Aim.Target = Vector3.new(-1.5, -0.05, 0.5)
		stress = 2
	else
		self.aim = false
		self.springs.Aim.Target = Vector3.zero
		stress = 0
	end
end
